...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) 
{
	...
	double A = -2 * (h2 + k2);
	double **v, **f, **u;
	double **vpreviter; //Вектор V предыдущей итерации
	double *x, *y;
	double v_old, v_new;
	double Eps_max = 0.0;
	...
	for (int j = 0; j <= m; j++)  //Заполнение граничных условий в массив v
	{
		v[0][j] = u1(1, y[j]);
		v[n][j] = u1(2, y[j]);
		vpreviter[0][j] = vpreviter[n][j] = 0.0;
	}
	...
	for (int i = 0; i <= n; i++)  //Заполнение граничных условий в массив v
	{
		v[i][0] = u1(x[i], 2);
		v[i][m] = u1(x[i], 3);
		vpreviter[i][0] = vpreviter[i][m] = 0.0;
	}
	...
	while (flag == false)       //Метод простой итерации
	{
		...
		for (int ind2 = 1; ind2 < m; ind2++)
		{
			...
			{
				vpreviter[ind1][ind2] = v[ind1][ind2];
			}
		}
		p++;
	...
	ref += "\r\n";
	ref += "Использовался метод простой итерации с параметрами Nmax=";
	sprintf_s(buffer, "%d", Nmax);
	ref += buffer;
	ref += ", E=";
	sprintf_s(buffer, "%.6e", Epsilon);
	ref += buffer;
	ref += ", оптимальный параметр tau=";
	sprintf_s(buffer, "%.6e", tau);
	ref += buffer;
	ref += "\r\n";
	ref += "Сетка по x: n=";
...
private: System::Void button2_Click(System::Object^  sender, System::EventArgs^  e) 
{
	...
	double A = -2 * (h2 + k2);
	double **v, **v2, **f;
	double **vpreviter, **vpreviter2; //Вектор V предыдущей итерации
	double *x, *y;
	double v_old, v_new;
	double Eps_max = 0.0, Eps_max2 = 0.0;
	double tau, tau2; //Параметр метода
	...
	for (int j = 0; j <= m; j++)  //Заполнение граничных условий в массив v
	{
		v2[0][j] = mu1(y[j]);
		v2[n][j] = mu2(y[j]);
		vpreviter2[0][j] = vpreviter2[n][j] = 0.0;
	}
	...
	for (int i = 0; i <= n; i++)  //Заполнение граничных условий в массив v
	{
		v2[i][0] = mu3(x[i]);
		v2[i][m] = mu4(x[i]);
		vpreviter2[i][0] = vpreviter2[i][m] = 0.0;
	}
	...
	while (flag == false)       //Метод простой итерации
	{
		...
		for (int j = 1; j < m; j++)
		{
			for (int i = 1; i < n; i++)
			{
				...
				if (i - 1 == 0)
				{
					if (j - 1 == 0)
					{
						R = A*v2[i][j] + f[i][j] + h2*(v2[i + 1][j] + v2[i - 1][j]) + k2*(v2[i][j + 1] + v2[i][j - 1]);					
					}
					else
					{
						R = A*v2[i][j] + f[i][j] + h2*(v2[i + 1][j] + v2[i - 1][j]) + k2*(v2[i][j + 1] + vpreviter2[i][j - 1]);				
					}
				}
				else
				{
					if (j - 1 == 0)
					{
						R = A*v2[i][j] + f[i][j] + h2*(v2[i + 1][j] + vpreviter2[i - 1][j]) + k2*(v2[i][j + 1] + v2[i][j - 1]);						
					}
					else
					{
						R = A*v2[i][j] + f[i][j] + h2*(v2[i + 1][j] + vpreviter2[i - 1][j]) + k2*(v2[i][j + 1] + vpreviter2[i][j - 1]);						
					}
				}
		...
		for (int ind2 = 1; ind2 < m; ind2++)
		{
			for (int ind1 = 1; ind1 < n; ind1++)
			{
				vpreviter2[ind1][ind2] = v2[ind1][ind2];
			}
		}
		p2++;
	...
	ref += buffer;
	ref += "\r\n";
	ref += "Оптимальный параметр tau2=";
	sprintf_s(buffer, "%.3e", tau2);
	ref += buffer;
	ref += "\r\n";
